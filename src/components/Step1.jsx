...
const Step1SchoolManagement = () => {
    ...
    const addData = async () => {
        if (fieldValue.trim() !== "") {
            try {
                await addDoc(collection(db, "schoolData"), {
                    field: fieldValue,
                });
    ...
    const deleteData = async (id) => {
        try {
            await deleteDoc(doc(db, "schoolData", id));
            fetchData(); // Refresh the data
        } catch (error) {
    ...
    const editData = async () => {
        if (fieldValue.trim() !== "" && editingId) {
            try {
                const docRef = doc(db, "schoolData", editingId);
    ...
    return (
        <Box
            sx={{
                ...
                alignItems: 'center',
                padding: '20px',
                marginTop: '0px'  // Adjust top margin to reduce space
            }}
        >
            <Paper sx={{ padding: '20px', width: '100%', maxWidth: '500px' }} elevation={3}> {/* Adjust margin here */}
...
export default Step1SchoolManagement;
